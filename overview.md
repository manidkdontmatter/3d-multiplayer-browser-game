...
- Engine/game separation: `src/engine/` provides capabilities (ECS, physics, networking, rendering, systems); `src/game/` provides archetype definitions, asset manifests, and game-specific behavior. The engine never imports from game.
...
- Shared schemas/protocol/helpers live in `src/engine/shared` (engine-level types) and `src/game/shared` (game-level archetype data).
- Server networking boundary is explicitly layered in `src/engine/server/net`:
...
- Client networking boundary is explicitly layered in `src/engine/client/runtime/network`:
...
- Server simulation state is BitECS-backed (`src/engine/server/ecs/SimulationEcs.ts`) with nengi replication bridged separately (`src/engine/server/netcode/NetReplicationBridge.ts`) and keyed by ECS eid.
- All entities (characters, items, abilities, doors, projectiles) are created by a single archetype system: JSON templates define component sets, the engine assembles entities from them. There are no separate "creator" systems for different entity types — in an ECS they are all just entities with different components.
- Core content definitions are data-driven via JSON archetype catalogs in `src/game/shared/archetypes/` (entity templates, starter items, platform definitions, ability defaults).
...
- `src/engine/client`: browser runtime + rendering (engine-level)
- `src/engine/server`: authoritative simulation + network server (engine-level)
- `src/engine/shared`: engine-level shared types, netcode schemas, deterministic math
- `src/game/shared`: game-level archetype definitions, asset manifests, game constants
