...
const PROJECTILE_TIMEOUT_MS = readEnvNumber("E2E_PROJECTILE_TIMEOUT_MS", 5000);
...
async function waitForVisibleProjectileState(page, timeoutMs) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const state = await readState(page);
    if (state?.mode === "connected" && Array.isArray(state.projectiles) && state.projectiles.length > 0) {
      return state;
    }
    await delay(40);
  }
  throw new Error("Timed out waiting for projectile replication after cast.");
}
...
async function main() {
  ...
  if (!serverAlreadyRunning) {
    ...
    console.log(`[projectile-fx] using existing server at ${SERVER_URL}`);
  }
  ...
  if (!clientAlreadyRunning) {
    ...
    console.log(`[projectile-fx] using existing client at ${CLIENT_URL}`);
  }
  ...
  try {
    ...
    await page.evaluate(() => {
      window.trigger_test_primary_action?.(3);
    });

    finalState = await waitForVisibleProjectileState(page, PROJECTILE_TIMEOUT_MS);
    await page.screenshot({ path: path.join(OUTPUT_DIR, "projectile-fx-check.png"), fullPage: true });
    ...
    if (hasFatalConsoleError) {
      throw new Error("Console contained runtime errors. See output/smoke/projectile-fx-check-console.json");
    }

    const count = Array.isArray(finalState?.projectiles) ? finalState.projectiles.length : 0;
    console.log(`[projectile-fx] PASS connected state with ${count} projectile(s) replicated`);
  } catch (error) {
    exitCode = 1;
    console.error("[projectile-fx] FAIL", error);
  } finally {
    if (page && !finalState) {
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "projectile-fx-check-state.json"),
      JSON.stringify(finalState ?? null, null, 2),
      "utf8"
    );
    fs.writeFileSync(
      path.join(OUTPUT_DIR, "projectile-fx-check-console.json"),
